...
<meta name="description"
	content="Yazılım Video, JSP - HTML5 - CSS3 - MySQL">
<meta name="author"
	content="Mimar Aslan, Computer Engineer, mimaraslan.com">
...
<body>
					...
					<article>
						<%
							...
							try {
								...
								rstVideoIzle = stmtVideoIzle
										.executeQuery("SELECT * FROM video WHERE kategoriid ='"
												+ request.getParameter("kategoriid")
												+ "' AND videosirasi ='"
												+ (Integer.parseInt(request
														.getParameter("videosirasi")))
												+ "' ORDER BY videosirasi ASC;");

								// SON VIDEONUN SAYISINI ALAN SORGU---BASI--- 
								Statement stmtVideoIzle1 = conVideoIzle.createStatement();
								ResultSet rstVideoIzle1 = stmtVideoIzle1
										.executeQuery("SELECT * FROM video WHERE kategoriid ='"
												+ request.getParameter("kategoriid") + "';");
						...
						<table>
							<tr>
								<td><iframe width="560" height="315"
										src="//<%=rstVideoIzle.getString("videogommekodu")%>"
						...
						<p>
							<a href="<%=rstVideoIzle.getString("videoadresi")%>"
								target="_blank">YouTube'da bu videoyu izlemek için tıklayın</a>
						</p>
						...
						<table width="100%" border="0">
							<tr>
								<td align="left">
									...
									%> <a
									href="izle.jsp?kategoriid=<%=rstVideoIzle.getInt("kategoriid")%>&videosirasi=<%=1%>">
										&Ouml;nceki Video<br> <%
 	int birOnceki = (Integer.parseInt(request
 						.getParameter("videosirasi")) - 1);
 				stmtVideoIzleBirOnceki = conVideoIzle.createStatement();
 				rstVideoIzleBirOnceki = stmtVideoIzleBirOnceki
 						.executeQuery("SELECT * FROM video WHERE kategoriid ='"
 								+ request.getParameter("kategoriid")
 								+ "' AND videosirasi ='"
 								+ birOnceki
 ...
 %> <a
									href="izle.jsp?kategoriid=<%=rstVideoIzle.getInt("kategoriid")%>&videosirasi=<%=rstVideoIzle.getInt("videosirasi") - 1%>">
										&Ouml;nceki Video<br> <%
 	int birOnceki = (Integer.parseInt(request.getParameter("videosirasi")) - 1);
 				...
 				while (rstVideoIzleBirOnceki.next()) {
 					out.print(birOnceki
 							+ ". "
 							+ rstVideoIzleBirOnceki.getString("videoadi"));
 				}
 %>
								</a> <%
 	}
 %>
								...
								<td>
									...
									<p><%=request.getParameter("videosirasi") + ". "
							+ rstVideoIzle.getString("videoadi")%>
										(<%=rstVideoIzle.getInt("videosirasi")%>/
										<%=siraVideoIzle%>)
									</p>

								</td>
								<td align="right"><a
									href="izle.jsp?kategoriid=<%=rstVideoIzle.getInt("kategoriid")%>&videosirasi=<%=rstVideoIzle.getInt(4) + 1%>">
										Sonraki Video<br> <%
 	int birSonraki = (Integer.parseInt(request
 					.getParameter("videosirasi")) + 1);
 			stmtVideoIzleBirSonraki = conVideoIzle.createStatement();
 			rstVideoIzleBirSonraki = stmtVideoIzleBirOnceki
 					.executeQuery("SELECT * FROM video WHERE kategoriid ='"
 							+ request.getParameter("kategoriid")
 							+ "' AND videosirasi ='" + birSonraki + "'");
 			while (rstVideoIzleBirSonraki.next()) {
 				out.print(birSonraki + ". "
 						+ rstVideoIzleBirSonraki.getString("videoadi"));
 			}
 %>
								</a></td>
							</tr>
	...
	<footer class="center part clearfix">
		<ul class="social column3 mright">
			<li><a href="https://www.linkedin.com/in/ramazan-kayi%C5%9F-0a2a286b?trk=nav_responsive_tab_profile_pic">linkedin</a></li>
			<li><a href="https://www.facebook.com/profile.php?id=1549936425">Facebook</a></li>
